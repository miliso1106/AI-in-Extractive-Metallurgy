...
const friendlyColumnDescriptions = {
  processName: 'Name of the metallurgical process.',
  condition_1_name: 'Actual label of the first process-specific operating parameter.',
  condition_1_value: 'Measured value of the first process-specific operating parameter.',
  condition_2_name: 'Actual label of the second process-specific operating parameter.',
  condition_2_value: 'Measured value of the second process-specific operating parameter.',
  condition_3_name: 'Actual label of the third process-specific operating parameter.',
  condition_3_value: 'Measured value of the third process-specific operating parameter.',
  condition_4_name: 'Actual label of the fourth process-specific operating parameter.',
  condition_4_value: 'Measured value of the fourth process-specific operating parameter.',
  oreGrade: 'Ore grade or feed grade used in the simplified sample dashboard.',
  temperature: 'Operating temperature of the process.',
  pressure: 'Operating pressure of the process.',
  leachingTime: 'Residence time or leaching duration.',
  recoveryRate: 'Percentage of valuable metal recovered.',
  efficiency: 'Overall operating efficiency of the process.',
  wasteGenerated: 'Waste generated by the process.',
  waterUsage: 'Water consumption of the process.',
  energyConsumption: 'Energy consumption of the process.',
  co2Emissions: 'Carbon dioxide emissions associated with the process.',
  status: 'Current process classification such as Optimal, Warning, or Alert.',
  sourcePdf: 'Source document from which the process data was compiled.',
  timestamp: 'Time or batch reference for the observation.',
  id: 'Row identifier.',
};
...
const statusCriteria = [
  {
    ...
    examples: 'condition_1 to condition_4',
    description: 'These are the process-specific variables such as ore grade, temperature, pH, pressure, reagent dosage, or leach time.',
  },
  {
    label: 'Performance Metrics',
    examples: 'recoveryRate, efficiency',
    description: 'These indicate how well the process is extracting metal and how effectively it is operating.',
  },
  {
    label: 'Environmental Metrics',
    examples: 'wasteGenerated, waterUsage, energyConsumption, co2Emissions',
    description: 'These indicate sustainability burden and help judge whether a process is acceptable in practice.',
  },
];

const getValueRangeLabel = (values) => {
...
const buildProcessParameterSummary = (rows) => {
  if (!rows?.length) return [];

  const grouped = new Map();
  rows.forEach((row) => {
    const processName = String(row.processName || row.process || 'Unknown Process').trim();
    if (!grouped.has(processName)) {
      grouped.set(processName, []);
    }

    for (let index = 1; index <= 4; index += 1) {
      const label = String(row[`condition_${index}_name`] ?? '').trim();
      const value = row[`condition_${index}_value`];
      if (!label) continue;

      const processConditions = grouped.get(processName);
      let existing = processConditions.find((entry) => entry.slot === `Condition ${index}`);
      if (!existing) {
        existing = {
          slot: `Condition ${index}`,
          label,
          values: [],
        };
        processConditions.push(existing);
      }
      existing.values.push(value);
    }
  });

  return Array.from(grouped.entries()).map(([processName, conditions]) => ({
    processName,
    conditions: conditions.map((condition) => ({
      slot: condition.slot,
      label: condition.label,
      range: getValueRangeLabel(condition.values),
    })),
  }));
};
...
const ProcessDataPage = () => {
  ...
  const [csvObjects, setCsvObjects] = useState([]);
  ...
  const displayedColumns = useMemo(() => {
    if (csvTable?.columns?.length) return csvTable.columns;
    if (!data.length) return [];
    return Object.keys(data[0]);
  }, [csvTable, data]);

  const parameterSummary = useMemo(
    () => buildProcessParameterSummary(csvObjects),
    [csvObjects],
  );
    ...
    reader.onload = () => {
      try {
        ...
        const rawObjects = dataRows.map((row) =>
          Object.fromEntries(rawHeaders.map((header, index) => [header, row[index] ?? '']))
        );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card">
        <div className="card-header">
          <h2 className="text-xl font-semibold text-white">Uploaded CSV Parameter Guide</h2>
          <p className="text-slate-400 text-sm mt-1">
            This section is for beginners. It shows all columns in the uploaded file, what they mean, and how the four condition slots map to real process parameters.
          </p>
        </div>
        <div className="card-body space-y-6">
          <div>
            <h3 className="text-lg font-semibold text-white mb-3">Columns Found In Current Dataset</h3>
            {displayedColumns.length ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {displayedColumns.map((column) => (
                  <div key={column} className="bg-slate-800 p-3 rounded border border-slate-700">
                    <p className="text-sm font-semibold text-white">{column}</p>
                    <p className="text-xs text-slate-400 mt-1">
                      {friendlyColumnDescriptions[column] || 'Custom column from the uploaded CSV.'}
                    </p>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-sm text-slate-400">Upload a CSV to inspect its columns.</p>
            )}
          </div>

          <div>
            <h3 className="text-lg font-semibold text-white mb-3">Process-wise Meaning Of Condition 1 To 4</h3>
            {parameterSummary.length ? (
              <div className="space-y-4">
                {parameterSummary.map((process) => (
                  <div key={process.processName} className="bg-slate-800 rounded border border-slate-700 p-4">
                    <p className="text-base font-semibold text-white mb-3">{process.processName}</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                      {process.conditions.map((condition) => (
                        <div key={`${process.processName}-${condition.slot}`} className="bg-slate-900/60 rounded p-3 border border-slate-700">
                          <p className="text-blue-300 font-medium">{condition.slot}</p>
                          <p className="text-white">{condition.label}</p>
                          <p className="text-xs text-slate-400 mt-1">Observed values: {condition.range}</p>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <p className="text-sm text-slate-400">
                If the uploaded CSV contains `condition_1_name` to `condition_4_name`, their meanings will be listed here process by process.
              </p>
            )}
          </div>

          <div>
            <h3 className="text-lg font-semibold text-white mb-3">What Should Be Used To Judge Status</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {statusCriteria.map((criterion) => (
                <div key={criterion.label} className="bg-slate-800 p-4 rounded border border-slate-700">
                  <p className="text-white font-semibold">{criterion.label}</p>
                  <p className="text-xs text-blue-300 mt-2">{criterion.examples}</p>
                  <p className="text-sm text-slate-400 mt-2">{criterion.description}</p>
                </div>
              ))}
            </div>
            <div className="mt-4 p-4 bg-slate-800 rounded border border-slate-700 text-sm text-slate-300">
              <p className="font-semibold text-white mb-2">Beginner Note</p>
              <p>
                `Optimal`, `Warning`, and `Alert` should ideally be judged from all major operating, recovery, and environmental parameters together, not from only one or two visible values.
              </p>
            </div>
          </div>
          ...
          {csvTable && (
            <div>
              <h3 className="text-lg font-semibold text-white mb-3">Full Uploaded CSV Preview</h3>
              <SimpleTable
                columns={csvTable.columns}
                rows={csvTable.rows.slice(0, 25)}
                emptyMessage="No uploaded CSV preview available."
              />
              <p className="text-xs text-slate-400 mt-2">
                Showing the first 25 rows for readability. The uploaded file contains {rowCount} rows.
              </p>
            </div>
          )}
        </div>
